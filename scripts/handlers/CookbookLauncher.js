...
/**
 * One delegated click handler opens the read-only viewer wherever a launcher
 * appears: the party journal entry, a shared chat card, or anywhere else. A single
 * document-level listener avoids depending on per-sheet render hooks.
 */
export const CookbookLauncher = {
    ...
    init() {
        document.addEventListener("click", (event) => {
            ...
            LivingCookbookApp.openReadOnly();
        });
    }
};
...
export async function ensurePartyCookbookJournal() {
    ...
    const content = `<p>The party's shared cookbook. Open it to review which creatures have been logged and which recipes are inscribed. Whoever carries the book does the cooking; this view is for everyone.</p>`
