...
async function resolveYieldCanonical(y) {
    ...
    try {
        const { ItemOutcomeHandler } = await import(
            "/modules/ionrift-respite/scripts/services/crafting/outcomes/ItemOutcomeHandler.js"
        );
...
/**
 * @param {{ actor?: Actor, actorUuid?: string, yields?: object[], creatureName?: string, tier?: string }} data
 */
export async function handleGrantYieldsRelay(data) {
    const actor = data.actor ?? (data.actorUuid ? await fromUuid(data.actorUuid) : null);
    if (!actor) {
        Logger.warn(`ItemFactory: actor not found for ${data.actorUuid}.`);
        return;
    }
    await grantYields(
        actor,
