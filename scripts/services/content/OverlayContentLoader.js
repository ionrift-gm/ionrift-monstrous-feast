...
import { buffRegistry } from "../meals/MealBuffHandlers.js";
import { ConsolePanelRegistry } from "../../ui/ConsolePanelRegistry.js";
import { MODULE_ID } from "../../data/moduleId.js";
const OVERLAY_ROOT = `ionrift-data/overlays/${MODULE_ID}`;

/** Always probe `core` even without the overlay API; other sublayers come from browse/API. */
const KNOWN_SUBLAYERS = ["core"];
...
/** Loads overlay creatures, recipes, buff plugins, and console panels. */
export class OverlayContentLoader {
    ...
    /** @type {Map<string, Set<string>>} Console panel ids keyed by overlay id. */
     ...
     * panel rides on the overlay content: present when the pack is available,
     ...
     * entry counts as active so manually placed overlay content remains usable.
     * @param {string} overlayId
     * @returns {boolean}
     * @private
     */
    static _isDeactivated(overlayId) {
        const map = Library.getWorldSetting("overlayWorldState", {});
        return map?.[overlayId]?.active === false;
    }

    /**
     * Read a JSON file from a sublayer. Prefers the library overlay API, then
     * falls back to a direct fetch. Returns null when the
