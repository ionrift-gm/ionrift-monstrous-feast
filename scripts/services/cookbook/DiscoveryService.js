...
export const DiscoveryService = {
    ...
    /**
     * @param {{ book?: Item, bookUuid?: string, typeId?: string }} data
     */
    async applyRelayedDiscovery(data) {
        const book = data.book ?? (data.bookUuid ? await fromUuid(data.bookUuid) : null);
        if (!book) {
            Logger.warn(`DiscoveryService: book not found for ${data.bookUuid}.`);
            return;
        }
        if (!data.typeId) return;
        const discovered = new Set(this.getDiscoveredTypes(book));
        if (discovered.has(data.typeId)) return;
        discovered.add(data.typeId);
        await book.setFlag(MODULE_ID, "discoveredTypes", [...discovered]);
    },

    /**
     * @param {{ book?: Item, bookUuid?: string, recipeId?: string }} data
     */
    async applyRelayedInscribe(data) {
        const book = data.book ?? (data.bookUuid ? await fromUuid(data.bookUuid) : null);
        if (!book) {
            Logger.warn(`DiscoveryService: book not found for ${data.bookUuid}.`);
            return;
        ...
        const inscribed = new Set(this.getInscribedRecipes(book));
        if (inscribed.has(data.recipeId)) return;
        inscribed.add(data.recipeId);
        await book.setFlag(MODULE_ID, "inscribedRecipes", [...inscribed]);
    },
